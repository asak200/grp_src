    ...
    : Node("aruco_tracker_node")
{
    ...
    // In OpenCV 4.5, DetectorParameters are managed via cv::Ptr
    _detectorParams = cv::aruco::DetectorParameters::create();

    // Fetch the predefined dictionary pointer
    _dictionary = cv::aruco::getPredefinedDictionary(_param_dictionary);

    auto qos = rclcpp::QoS(1).best_effort();

    _image_sub = create_subscription<sensor_msgs::msg::Image>(
                 "/camera/image_raw", qos, std::bind(&ArucoTrackerNode::image_callback, this, std::placeholders::_1));
	...
	RCLCPP_INFO(get_logger(), "aruco_tracker_node initialized");
...
void ArucoTrackerNode::image_callback(const sensor_msgs::msg::Image::SharedPtr msg)
{
    try {
        ...
        // Detect markers using the OpenCV 4.5 global method
        std::vector<int> ids;
